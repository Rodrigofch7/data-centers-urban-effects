...
def merging():
    ...
    housing_path = pathlib.Path("zillow_yearly_estimates_cook_county.csv")
    ...
    housing_long = housing_long.sort_values(["ZipCode","year"])
    housing_long["hval_yrbefore"] = (housing_long.groupby("ZipCode")["hval"].shift(1))
    housing_long["hval_yrafter"] = (housing_long.groupby("ZipCode")["hval"].shift(-1))
    ...
    merged = datacenters.merge(housing_long, 
                               left_on=["Zipcode", "first_permit"],
                               right_on=["ZipCode", "year"], 
                               how="left")

    #Removing duplicate columns and rearranging
    merged = merged.drop(columns=["ZipCode", "first_permit","StateName"])
