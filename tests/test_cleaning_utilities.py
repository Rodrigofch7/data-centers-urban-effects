...
from data_centers_next_door.hc_and_utility_scores.cleaning_utilities import (
    rename_dfcols,
    calculate_elecScore,
    consolidate_hhc_2000_plus,
)


@pytest.fixture
def loading_data():
    elec_water = pd.read_csv(
        Path("data/clean_elecwater_hc_scores/elec_water_cleaned.csv"), dtype={"ZCTA5A": str}
    )
    monthHHC = pd.read_csv(
