...
def datacenters_vis_zipcode():
    ...
    unique_dc_count = (
        datacenters_housing.groupby("Zipcode")["Address"]
        .nunique()
        .reset_index(name="Data Center Count")
    )
    # In mark_bar, the color and outline of the bar is added
    chart = (
        alt.Chart(unique_dc_count)
        .mark_bar(color=MAROON, stroke=TEXT_ACC, strokeWidth=0.5)
        .encode(
            x=alt.X(
                "Data Center Count:Q", title="Number of Data Centers", axis=alt.Axis(tickMinStep=1)
            ),
            y=alt.Y("Zipcode:N", sort="-x", title="ZIP Code"),
            tooltip=["Data Center Count"],
        )
        .properties(
            title={"text": f"Number of Data Centers in Each Zip Code", "color": TEXT_PRI},
            background=DARK_BG,
        )
        .configure_axis(labelColor=TEXT_SEC, titleColor=TEXT_PRI)
        .configure_title(font="Helvetica", anchor="start")
        .interactive()
    )
...
def datacenters_vis_company():
    ...
    datacenters_housing = pd.read_csv(FILENAME, dtype={"Zipcode": str})
    unique_dc_count = (
        datacenters_housing.groupby("Operator")["Address"]
        .nunique()
        .reset_index(name="Data Center Count")
    )
    chart = (
        ...
        .properties(
            title={"text": f"Number of Data Centers by Each Company", "color": TEXT_PRI},
            background=DARK_BG,
        )
        .configure_axis(labelColor=TEXT_SEC, titleColor=TEXT_PRI)
        .configure_title(fontSize=18, font="Helvetica", anchor="start")
        .interactive()
    )
