...
chicago_data_centers["First_Operation_Permit"] = pd.to_datetime(chicago_data_centers["First_Operation_Permit"], format="%Y")
# Gathering the cumuluative count of data centers over time
chicago_data_centers_by_year = (chicago_data_centers["First_Operation_Permit"].value_counts().sort_index().cumsum().reset_index())

#### Creating the time series chart 
time_series = (alt.Chart(chicago_data_centers_by_year)
               .mark_line()
               .encode(x = alt.X("First_Operation_Permit:T",axis = alt.Axis(title = "Year")),
